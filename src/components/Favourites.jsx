  ...
  return (
    <>
      ...
      <div className='mt-4 flex space-x-2 justify-center flex-wrap'> 
        {genres.map((genre) => {
          return(
            <button className='p-1 px-2 bg-gray-300 rounded-lg text-lg font-bold text-white hover:bg-blue-300 my-2 mx-2'>{genre}</button>
          )
        })}
      ...
      <div className='mt-4 flex justify-center space-x-2'>
        <input type="text" value={searchItem} onChange={(e) => {setSearchItem(e.target.value)}} placeholder='Search' className='border-2 py-1 px-2 placeholder:text-center focus:outline-none focus:ring-2 focus:ring-blue-300 focus:border-transparent' />
    ...
    <tbody className="divide-y divide-gray-100 border-t border-gray-100">
      {searchedMovies.map((movie) => {
        ...
        <th className="flex items-center px-6 py-4 font-normal text-gray-900 space-x-2">
            ...
            />
            <div className="font-medium text-gray-700 justify-center">{movie.title || movie.name}</div>
        </th>
        ...
        <td className="px-6 py-4">
          <div className='flex justify-center'>
          <span onClick={() => {deleteMovie(movie.id)}} className="inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs font-semibold text-red-600 cursor-pointer">
              Delete
            </span>
          </div>
        </td>
